...
mod tests {
    ...
    async fn insert_collection_hnsw_index(
        ...
        for (doc_id, obj) in collection.to_vec_ref() {
            ...
            h.index_document(tx, Id::Number(*doc_id as i64), &content)
                .await
                .unwrap();
    ...
    async fn delete_hnsw_index_collection(
        ...
        for (doc_id, obj) in collection.to_vec_ref() {
            let content = vec![Value::from(obj.deref())];
            h.remove_document(tx, Id::Number(*doc_id as i64), &content)
                .await?;
    ...
    async fn test_hnsw_index(collection_size: usize, unique: bool, p: HnswParams) {
        ...
        let (mut h, map) = {
            ...
            let tx = ctx.tx();
            let mut h = HnswIndex::new(&tx, IndexKeyBase::default(), "test".to_string(), &p)
                .await
                .unwrap();
            // Fill index
