...
impl Document {
            ...
            Force::Table(tb)
                if tb.first().is_some_and(|tb| {
                    tb.view.as_ref().is_some_and(|v| {
                        self.id
                            .as_ref()
                            .is_some_and(|id| v.what.iter().any(|p| p.0 == id.tb))
                    })
                }) =>
            {
                tb.clone()
        ...
        Ok(stk
            .scope(|scope| {
                try_join_all(
                    group
                        .iter()
                        .map(|v| scope.run(|stk| v.compute(stk, ctx, opt, Some(doc)))),
                )
            })
            .await?
            .into_iter()
    ...
    async fn full(
        ...
        let mut data = exp
            .compute(stk, ctx, opt, Some(&self.current), false)
            .await?;
        data.cut(ID.as_ref());
    ...
    async fn fields(
        ...
        for field in fdc.view.expr.other() {
            // Process the field
            if let Field::Single { expr, alias } = field {
    ...
    fn one_group_query(
        ...
        Ok(Value::Subquery(Box::new(Subquery::Ifelse(
            IfelseStatement {
                exprs: vec![(
                    Value::Expression(Box::new(Expression::Binary {
                        l: Value::Idiom(key.clone()),
