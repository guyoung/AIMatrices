...
pub(crate) fn default_headers() -> HeaderMap {
    ...
    headers.insert(
        CONTENT_TYPE,
        HeaderValue::from_static("application/surrealdb"),
    );
    headers
}
...
impl Authenticate for RequestBuilder {
    fn auth(self, auth: &Option<Auth>) -> Self {
        match auth {
            Some(Auth::Basic { user, pass, ns, db }) => {
...
async fn router(
    ...
    match req.command {
        Command::Query {
            ...
            let req = Command::Query { query, variables }
        ...
        Command::Signin { credentials } => {
            ...
            if let Ok(Credentials { user, pass, ns, db }) = from_core_value(credentials.into()) {
                *auth = Some(Auth::Basic { user, pass, ns, db });
            } else {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        Command::ExportFile { path, config } => {
        ...
        Command::SubscribeLive { .. } => Err(Error::LiveQueriesNotSupported.into()),
        cmd => {
