...
impl IndexStores {
    pub async fn get_store_btree_fst(
        ...
        let cache = self
            .0
            .btree_fst_caches
            .get_cache(generation, &keys, cache_size)
            .await;
    ...
    pub(crate) async fn index_removed(
        ...
        self.remove_index(ns, db, tx.get_tb_index(ns, db, tb, ix).await?.as_ref())
            .await
    }
    ...
    fn remove_search_caches(&self, ikb: IndexKeyBase) {
        self.0
            .btree_trie_caches
            .remove_caches(&TreeNodeProvider::DocIds(ikb.clone()));
        self.0
            .btree_trie_caches
            .remove_caches(&TreeNodeProvider::DocLengths(ikb.clone()));
        self.0
            .btree_trie_caches
            .remove_caches(&TreeNodeProvider::Postings(ikb.clone()));
        self.0
            .btree_fst_caches
            .remove_caches(&TreeNodeProvider::Terms(ikb));
    }
    ...
    fn remove_mtree_caches(&self, ikb: IndexKeyBase) {
        ...
        self.0
            .mtree_caches
            .remove_caches(&TreeNodeProvider::Vector(ikb.clone()));
    }
