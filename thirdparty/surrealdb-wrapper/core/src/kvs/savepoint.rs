...
impl SavePoints {
    ...
    pub(super) fn is_saved_key(&self, key: &Key) -> Option<bool> {
        self.current
            .as_ref()
            .map(|current| current.contains_key(key))
    }
            ...
            match saved_value.last_operation {
                SaveOperation::Set | SaveOperation::Put => {
                    if let Some(initial_value) = saved_value.saved_val {
                        ...
                        tx.set(key, initial_value, saved_value.saved_version)
                            .await?;
                    } else {
                ...
                SaveOperation::Del => {
                    if let Some(initial_value) = saved_value.saved_val {
                        ...
                        tx.put(key, initial_value, saved_value.saved_version)
                            .await?;
                    }
                }
            }
    ...
    async fn save_point_prepare(
        ...
        let r = match is_saved_key {
            ...
            Some(false) => {
                ...
                Some(SavePrepare::NewKey(
                    key.clone(),
                    SavedValue::new(val, version, op),
                ))
            }
        };
        Ok(r)
