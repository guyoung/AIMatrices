...
impl Model {
    ...
    pub(crate) async fn compute(
        ...
        let val = ctx
            .tx()
            .get_db_model(opt.ns()?, opt.db()?, &self.name, &self.version)
            .await?;
        ...
        let mut args = stk
            .scope(|stk| {
                try_join_all(
                    self.args
                        .iter()
                        .map(|v| stk.run(|stk| v.compute(stk, ctx, opt, doc))),
                )
            })
            .await?;
        ...
        match args.swap_remove(0) {
            ...
            Value::Object(v) => {
                ...
                let outcome: Vec<f32> = tokio::task::spawn_blocking(move || {
                    ...
                    compute_unit
                        .buffered_compute(&mut args)
                        .map_err(|err: SurrealError| {
                            Error::ModelComputation(err.message.to_string())
                        })
                })
                .await
