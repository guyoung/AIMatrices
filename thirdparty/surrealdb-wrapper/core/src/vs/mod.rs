...
impl Iterator for VersionstampSequence {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let index_to_increase = next_state
            .iter()
            .enumerate()
            .rev()
            .skip(2)
        ...
        for next_state_byte in next_state
            .iter_mut()
            .take(returned_state.len() - 2)
            .skip(index_to_increase + 1)
        {
            *next_state_byte = 0;
        }
...
mod test {
    ...
    pub fn generate_one_vs() {
        let vs = super::generate_versionstamp_sequences([0; 10])
            .take(1)
            .collect::<Vec<_>>();
