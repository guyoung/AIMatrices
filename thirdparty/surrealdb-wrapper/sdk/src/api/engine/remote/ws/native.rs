...
pub(crate) async fn connect(
    ...
    request.headers_mut().insert(
        ...
        HeaderValue::from_static(super::REVISION_HEADER),
    );
...
async fn router_handle_route(
    ...
    let RequestData { id, command } = request;
    ...
    match command {
        ...
        Command::Unset { ref key } => {
            effect = RequestEffect::Clear { key: key.clone() };
        }
        Command::Insert { .. } => {
        ...
        Command::SubscribeLive {
            ...
            state
                .live_queries
                .insert(*uuid, notification_sender.clone());
            if response
                .clone()
                .send(Ok(DbResponse::Other(CoreValue::None)))
                .await
                .is_err()
            {
                trace!("Receiver dropped");
        ...
        Command::Invalidate { .. } => {
            state
                .replay
                .insert(ReplayMethod::Invalidate, command.clone());
        }
        Command::Authenticate { .. } => {
            state
                .replay
                .insert(ReplayMethod::Authenticate, command.clone());
        }
        _ => {}
    }
    ...
    match Response::try_from(&response) {
                        ...
                        if let Ok(id) = id.coerce_to_i64() {
                            if let Some(pending) = state.pending_requests.remove(&id) {
                                ...
                                match pending.effect {
                                    ...
                                    RequestEffect::Set { key, value } => {
                                        state.vars.insert(key, value);
                                    }
                                    RequestEffect::Clear { key } => {
        ...
        Err(error) => {
            ...
            if let Message::Binary(binary) = response {
                if let Ok(ErrorResponse { id }) = deserialize(&binary, true) {
