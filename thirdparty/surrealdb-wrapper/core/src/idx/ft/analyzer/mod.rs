...
impl Analyzer {
    ...
    pub(super) async fn extract_querying_terms(
        ...
        let tokens = self
            .generate_tokens(stk, ctx, opt, FilteringStage::Querying, content)
            .await?;
    ...
    pub(in crate::idx) async fn extract_indexing_terms(
        ...
        let mut tv = Vec::new();
        self.analyze_value(stk, ctx, opt, content, FilteringStage::Indexing, &mut tv)
            .await?;
        let mut set = HashSet::new();
        let mut has_unknown_terms = false;
        let tx = ctx.tx();
    ...
    async fn analyze_value(
        ...
        match val {
            ...
            Value::Number(n) => tks.push(
                self.generate_tokens(stk, ctx, opt, stage, n.to_string())
                    .await?,
            ),
            Value::Bool(b) => tks.push(
                self.generate_tokens(stk, ctx, opt, stage, b.to_string())
                    .await?,
            ),
            Value::Array(a) => {
                for v in a.0 {
