    ...
    fn try_from(val: Value) -> Result<Self, &'static str> {
        match val {
            ...
            Value::Number(v) => match v {
                ...
                Number::Decimal(v) => Ok(Pack(Data::Ext(
                    TAG_DECIMAL,
                    v.to_string().as_bytes().to_vec(),
                ))),
            },
            Value::Strand(v) => Ok(Pack(Data::String(v.0.into()))),
            Value::Duration(v) => Ok(Pack(Data::Ext(
                TAG_DURATION,
                v.to_raw().as_bytes().to_vec(),
            ))),
            Value::Datetime(v) => Ok(Pack(Data::Ext(
                TAG_DATETIME,
                v.to_raw().as_bytes().to_vec(),
            ))),
            ...
            Value::Thing(v) => Ok(Pack(Data::Ext(
                TAG_RECORDID,
                v.to_raw().as_bytes().to_vec(),
            ))),
