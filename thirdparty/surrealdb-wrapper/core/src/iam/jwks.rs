...
static CACHE_EXPIRATION: LazyLock<chrono::Duration> =
    LazyLock::new(
        || match std::env::var("SURREAL_JWKS_CACHE_EXPIRATION_SECONDS") {
            Ok(seconds_str) => {
...
static CACHE_COOLDOWN: LazyLock<chrono::Duration> =
    LazyLock::new(
        || match std::env::var("SURREAL_JWKS_CACHE_COOLDOWN_SECONDS") {
            Ok(seconds_str) => {
                let seconds = seconds_str.parse::<u64>().expect(
...
static REMOTE_TIMEOUT: LazyLock<chrono::Duration> =
    LazyLock::new(
        || match std::env::var("SURREAL_JWKS_REMOTE_TIMEOUT_MILLISECONDS") {
            Ok(milliseconds_str) => {
...
pub(super) async fn config(
    ...
    let alg = match jwk.common.key_algorithm {
        Some(alg) => match alg {
            ...
            _ => {
                warn!(
                    "Unspported value for parameter 'alg' in JWK object: '{:?}'",
                    alg
                );
                return Err(Error::InvalidAuth); // Return opaque error
            }
...
async fn find_jwk_from_url(kvs: &Datastore, url: &str, kid: &str) -> Result<Jwk, Error> {
    ...
    match fetch_jwks_from_url(cache, url).await {
        ...
        Err(err) => {
            warn!(
                "Failed to fetch JWKS object from remote location: '{}'",
                err
            );
            Err(Error::InvalidAuth) // Return opaque error
        }
...
async fn fetch_jwks_from_url(cache: &Arc<RwLock<JwksCache>>, url: &str) -> Result<JwkSet, Error> {
    ...
    let res = client
        .get(url)
        .timeout((*REMOTE_TIMEOUT).to_std().unwrap())
        .send()
        .await?;
...
mod tests {
    ...
    async fn test_golden_path() {
        ...
        assert!(
            res.is_ok(),
            "Failed to validate token the first time: {:?}",
            res.err()
        );
        ...
        assert!(
            res.is_ok(),
            "Failed to validate token the second time: {:?}",
            res.err()
        );
    }
    ...
    async fn test_capabilities_default() {
        ...
        assert!(
            res.is_err(),
            "Unexpected success validating token from unallowed remote location"
        );
    }
    ...
    async fn test_cache_cooldown() {
        ...
        assert!(
            res.is_err(),
            "Unexpected success validating token with invalid key identifier"
        );
