...
impl TryFrom<Val> for OffsetRecords {
    ...
    fn try_from(val: Val) -> Result<Self, Self::Error> {
        ...
        let n_offsets = *decompressed
            .first()
            .ok_or(Error::CorruptedIndex("OffsetRecords::try_from(1)"))?
            as usize;
        ...
        for _ in 0..n_offsets {
            let index = indexes
                .next()
                .ok_or(Error::CorruptedIndex("OffsetRecords::try_from(2)"))?;
            let start = tail
                .next()
                .ok_or(Error::CorruptedIndex("OffsetRecords::try_from(3)"))?;
            let gen_start = if without_gen_start {
                start
            } else {
                tail.next()
                    .ok_or(Error::CorruptedIndex("OffsetRecords::try_from(4)"))?
            };
            let end = tail
                .next()
                .ok_or(Error::CorruptedIndex("OffsetRecords::try_from(5)"))?;
