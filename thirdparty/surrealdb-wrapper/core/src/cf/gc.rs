...
pub async fn gc_all_at(tx: &Transaction, ts: u64) -> Result<(), Error> {
    ...
    for ns in nss.as_ref() {
        ...
        // Trace for debugging
        trace!(
            "Performing garbage collection on {} for timestamp {ts}",
            ns.name
        );
...
pub async fn gc_ns(tx: &Transaction, ts: u64, ns: &str) -> Result<(), Error> {
    ...
    for db in dbs.as_ref() {
        ...
        // Trace for debugging
        trace!(
            "Performing garbage collection on {ns}:{} for timestamp {ts}",
            db.name
        );
        // Fetch all tables
        ...
        let watermark_vs = tx
            .lock()
            .await
            .get_versionstamp_from_timestamp(watermark_ts, ns, &db.name)
            .await?;
