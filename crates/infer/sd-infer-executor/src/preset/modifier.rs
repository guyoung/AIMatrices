...
pub fn real_esrgan_x4plus_anime_6_b(
    mut builder: ContextConfigBuilder,
...
pub fn hybrid_taesd(mut builder: ContextConfigBuilder) -> anyhow::Result<ContextConfigBuilder> {
    let taesd_path =
        get_huggingface_model_path("cqyan/hybrid-sd-tinyvae/diffusion_pytorch_model.safetensors");
    builder.taesd(taesd_path);
    Ok(builder)
}
...
pub fn t5xxl_fp16_flux_1(
    mut builder: ContextConfigBuilder,
) -> anyhow::Result<ContextConfigBuilder> {
    let t5xxl_path =
        get_huggingface_model_path("comfyanonymous/flux_text_encoders/t5xxl_fp16.safetensors");
