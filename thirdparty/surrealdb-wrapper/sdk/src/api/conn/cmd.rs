...
impl Command {
    ...
    pub(crate) fn into_router_request(self, id: Option<i64>) -> Option<RouterRequest> {
        let res = match self {
            ...
            Command::Signup { credentials } => RouterRequest {
                id,
                method: "signup",
                params: Some(vec![CoreValue::from(credentials)].into()),
            },
            Command::Signin { credentials } => RouterRequest {
                id,
                method: "signin",
                params: Some(vec![CoreValue::from(credentials)].into()),
            },
            Command::Authenticate { token } => RouterRequest {
                id,
                method: "authenticate",
            ...
            Command::Select { what, .. } => RouterRequest {
                id,
                method: "select",
                params: Some(CoreValue::Array(vec![what.into_core_value()].into())),
            },
            Command::Delete { what, .. } => RouterRequest {
                id,
                method: "delete",
                params: Some(CoreValue::Array(vec![what.into_core_value()].into())),
            },
            ...
            Command::Set { key, value } => RouterRequest {
                id,
                method: "let",
                params: Some(CoreValue::from(vec![CoreValue::from(key), value])),
            },
            Command::Unset { key } => RouterRequest {
                id,
                method: "unset",
                params: Some(CoreValue::from(vec![CoreValue::from(key)])),
            },
            Command::SubscribeLive { .. } => return None,
            Command::Kill { uuid } => RouterRequest {
                id,
                method: "kill",
    ...
    pub(crate) fn needs_flatten(&self) -> bool {
        match self {
            ...
            | Command::Merge { what, .. }
            | Command::Select { what }
            | Command::Delete { what } => matches!(what, Resource::RecordId(_)),
            Command::Insert { data, .. } => !data.is_array(),
            _ => false,
        }
    }
}
...
mod test {
        ...
        D: FnOnce(I) -> Value,
    {
        ...
        assert_eq!(
            obj.get("id")
                .cloned()
                .and_then(|x| if let Value::Number(Number::Int(x)) = x {
                    Some(x)
                } else {
                    None
                }),
