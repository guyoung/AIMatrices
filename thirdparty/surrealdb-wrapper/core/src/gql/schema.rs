...
pub async fn generate_schema(
    ...
    let cg = tx
        .get_db_config(ns, db, "graphql")
        .await
        .map_err(|e| match e {
            crate::err::Error::CgNotFound { .. } => GqlError::NotConfigured,
            e => e.into(),
        })?;
    ...
    let tbs = match config.tables {
        ...
        TablesConfig::Include(inc) => tbs
            .iter()
            .filter(|t| inc.contains_name(&t.name))
            .cloned()
            .collect(),
        TablesConfig::Exclude(exc) => tbs
            .iter()
            .filter(|t| !exc.contains_name(&t.name))
            .cloned()
            .collect(),
    };
    ...
    for tb in tbs.iter() {
        ...
        let table_order = InputObject::new(&table_order_name)
            ...
            .field(InputValue::new(
                "asc",
                TypeRef::named(&table_orderable_name),
            ))
            .field(InputValue::new(
                "desc",
                TypeRef::named(&table_orderable_name),
        ...
        table_filter = table_filter
            ...
            .field(InputValue::new(
                "and",
                TypeRef::named_nn_list(&table_filter_name),
            ))
            .field(InputValue::new(
                "or",
                TypeRef::named_nn_list(&table_filter_name),
        ...
        for fd in fds.iter() {
            ...
            table_filter = table_filter.field(InputValue::new(
                fd.name.to_string(),
                TypeRef::named(type_filter_name),
            ));
...
pub fn sql_value_to_gql_value(v: SqlValue) -> Result<GqlValue, GqlError> {
    let out = match v {
        ...
        SqlValue::Number(n) => match n {
            ...
            crate::sql::Number::Float(f) => GqlValue::Number(Number::from_f64(f).ok_or(
                resolver_error("unimplemented: graceful NaN and Inf handling"),
            )?),
        ...
        SqlValue::Array(a) => GqlValue::List(
            a.into_iter()
        ...
        v => {
            return Err(internal_error(format!(
                "found unsupported value variant: {v:?}"
            )))
        }
    };
    Ok(out)
...
fn kind_to_type(kind: Kind, types: &mut Vec<Type>) -> Result<TypeRef, GqlError> {
    ...
    let out_ty = match match_kind {
        ...
        Kind::Either(ks) => {
            let (ls, others): (Vec<Kind>, Vec<Kind>) = ks
                .into_iter()
                .partition(|k| matches!(k, Kind::Literal(Literal::String(_))));
    ...
    let ty = match &kind {
        Kind::Record(ts) => match ts.len() {
            1 => TypeRef::named(filter_name_from_table(
                ts.first()
                    .expect("ts should have exactly one element")
                    .as_str(),
            )),
...
fn negate(filter: &GqlValue, fds: &[DefineFieldStatement]) -> Result<SqlValue, GqlError> {
    let obj = filter
        .as_object()
        .ok_or(resolver_error("Value of NOT must be object"))?;
...
fn aggregate(
    ...
    let list = filter.as_list().ok_or(resolver_error(format!(
        "Value of {op_str} should be a list"
    )))?;
    let filter_arr = list
        .iter()
        .map(|v| v.as_object().map(|o| val_from_filter(o, fds)))
        .collect::<Option<Result<Vec<SqlValue>, GqlError>>>()
        .ok_or(resolver_error(format!(
            "List of {op_str} should contain objects"
        )))??;

    let mut iter = filter_arr.into_iter();

    let mut cond = iter.next().ok_or(resolver_error(format!(
...
fn binop(
    ...
    let obj = val
        .as_object()
        .ok_or(resolver_error("Field filter should be object"))?;
...
macro_rules! either_try_kind {
    ($ks:ident, $val:expr, Kind::Array) => {
        for arr_kind in $ks
            .iter()
            .filter(|k| matches!(k, Kind::Array(_, _)))
            .cloned()
        {
    ...
    match kind {
        ...
        Kind::Either(ref ks) => {
            ...
            match val {
                ...
                GqlValue::Binary(_) => Err(resolver_error(
                    "binary input for Either is not yet supported",
                )),
                GqlValue::Enum(n) => {
