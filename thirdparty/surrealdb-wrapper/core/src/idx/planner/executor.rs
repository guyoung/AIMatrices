...
impl InnerQueryExecutor {
    ...
    pub(super) async fn new(
                        ...
                        if let Some(e) = ft_entry {
                            if let Matches(_, Some(mr)) = e.0.index_option.op() {
                                if mr_entries.insert(*mr, e.clone()).is_some() {
                                    return Err(Error::DuplicatedMatchRef { mr: *mr });
                                }
                            }
        ...
        for (exp, knn) in kbtes {
            knn_bruteforce_entries.insert(
                exp,
                (KnnPriorityList::new(knn.k as usize), knn.id, knn.obj, knn.d),
            );
        }
...
impl QueryExecutor {
    ...
    async fn new_single_iterator(
        ...
        if let Some(ix) = self.get_index_def(io.ix_ref()) {
            match ix.index {
                Index::Idx => Ok(self
                    .new_index_iterator(opt, irf, ix.clone(), io.clone())
                    .await?),
                Index::Uniq => Ok(self
                    .new_unique_index_iterator(opt, irf, ix, io.clone())
                    .await?),
                Index::Search { .. } => self.new_search_index_iterator(irf, io.clone()).await,
            ...
            IndexOperator::Equality(value) | IndexOperator::Exactness(value) => {
                if let Value::Number(n) = value.as_ref() {
                    ...
                    if values.len() == 1 {
                        ...
                        Some(Self::new_multiple_index_equal_iterators(
                            irf, opt, &ix, values,
                        )?)
                    }
                } else {
    ...
    fn new_multiple_index_equal_iterators(
        ...
        Ok(ThingIterator::Multiples(Box::new(MultipleIterators::new(
            iterators,
        ))))
    }
    ...
    fn get_equal_number_variants(n: &Number) -> Vec<Value> {
        let (oi, of, od) =
            Self::get_number_variants(n, |f| if f.trunc().eq(f) { f.to_i64() } else { None });
    ...
    fn get_ranges_variants<'a>(
        ...
        match (&from.value, &to.value) {
            (Value::Number(from_n), Value::Number(to_n)) => Some(Self::get_ranges_number_variants(
                from_n,
                from.inclusive,
                to_n,
                to.inclusive,
            ...
            IndexOperator::Equality(value) | IndexOperator::Exactness(value) => {
                if let Value::Number(n) = value.as_ref() {
                    ...
                    if values.len() == 1 {
                        ...
                        Some(Self::new_multiple_unique_equal_iterators(
                            irf, opt, ix, values,
                        )?)
                    }
                } else {
    ...
    async fn matches_with_value(
        ...
        let t =
            ft.0.analyzer
                .extract_indexing_terms(stk, ctx, opt, &terms, v)
                .await?;
        drop(terms);
    ...
    pub(crate) async fn offsets(
        ...
        if let Some((e, ft)) = self.get_ft_entry_and_index(&match_ref) {
            let tx = ctx.tx();
            let res = ft
                .extract_offsets(&tx, thg, &e.0.query_terms_list, partial)
                .await;
            return res;
        }
        Ok(Value::None)
