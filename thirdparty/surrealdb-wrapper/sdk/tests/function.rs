...
async fn error_on_invalid_function() -> Result<(), Error> {
    ...
    query.0 .0 = vec![sql::Statement::Value(Value::Function(Box::new(
...
async fn function_math_log() -> Result<(), Error> {
    ...
    Test::new(sql)
        .await?
        .expect_floats(
            &[0.43429738512450866, 0.30102999566398114, 0.0],
            2.0 * f64::EPSILON,
        )?
...
async fn function_math_log10() -> Result<(), Error> {
    ...
    Test::new(sql)
        .await?
        .expect_floats(
            &[0.43429738512450866, std::f64::consts::LOG10_2, 0.0],
            f64::EPSILON,
        )?
...
async fn function_math_log2() -> Result<(), Error> {
    ...
    Test::new(sql)
        .await?
        .expect_floats(&[1.4427046851812222, 1.0, 0.0], f64::EPSILON)?;
    Ok(())
}
...
async fn function_math_rad2deg() -> Result<(), Error> {
    ...
    Test::new(sql)
        .await?
        .expect_vals(&["45f", "-90.0f", "360f", "180f"])?;
    Ok(())
}
...
async fn function_search_analyzer_invalid_arguments() -> Result<(), Error> {
    ...
    match test.next()?.result {
        Err(Error::InvalidArguments { name, message }) => {
...
async fn function_search_analyzer_invalid_return_type() -> Result<(), Error> {
    ...
    //
    match test.next()?.result {
        Err(Error::InvalidFunction { name, message }) => {
...
async fn function_type_decimal() -> Result<(), Error> {
    ...
    let val = Value::Number(Number::Decimal(
        "13.571938471938471938563985639413947693775639"
            .parse()
            .unwrap(),
    ));
    assert_eq!(tmp, val);
...
pub async fn function_http_put() -> Result<(), Error> {
    ...
    let query = format!(
        r#"RETURN http::put("{}/some/path",{{ 'some-key': 'some-value' }})"#,
        server.uri()
    );
...
async fn function_custom_optional_args() -> Result<(), Error> {
    ...
    match test.next()?.result {
        Err(surrealdb::error::Db::InvalidArguments { name, message })
