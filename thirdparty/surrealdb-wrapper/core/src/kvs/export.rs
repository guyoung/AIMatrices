...
impl Transaction {
    ...
    async fn export_metadata(
        ...
        // Output OPTIONS
        self.export_section("OPTION", vec!["OPTION IMPORT"], chn)
            .await?;
        ...
        if cfg.accesses {
            ...
            self.export_section("ACCESSES", accesses.to_vec(), chn)
                .await?;
        }
        ...
        if cfg.functions {
            ...
            self.export_section("FUNCTIONS", functions.to_vec(), chn)
                .await?;
        }
        ...
        if cfg.analyzers {
            ...
            self.export_section("ANALYZERS", analyzers.to_vec(), chn)
                .await?;
        }
    ...
    async fn export_table_structure(
        ...
        chn.send(bytes!("-- ------------------------------"))
            .await?;
        chn.send(bytes!(format!("-- TABLE: {}", table.name)))
            .await?;
        chn.send(bytes!("-- ------------------------------"))
            .await?;
        chn.send(bytes!("")).await?;
    ...
    async fn export_table_data(
        ...
        chn.send(bytes!("-- ------------------------------"))
            .await?;
        chn.send(bytes!(format!("-- TABLE DATA: {}", table.name)))
            .await?;
        chn.send(bytes!("-- ------------------------------"))
            .await?;
        chn.send(bytes!("")).await?;
