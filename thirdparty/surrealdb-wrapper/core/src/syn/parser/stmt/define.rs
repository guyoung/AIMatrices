            ...
            match self.peek_kind() {
                ...
                t!("THEN") => {
                    ...
                    while self.eat(t!(",")) {
                        res.then
                            .0
                            .push(ctx.run(|ctx| self.parse_value_field(ctx)).await?)
                    }
                }
                t!("COMMENT") => {
                    self.pop_peek();
        ...
        loop {
            match self.peek_kind() {
                x if Self::kind_is_identifier(x) => {
                    ...
                    acc.push(TableConfig { name: name.0 });
                }
