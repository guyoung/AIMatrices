...
impl FtIndex {
    ...
    pub(crate) async fn index_document(
        ...
        if self.highlighting {
            ...
            if let Some(old_term_ids) = old_term_ids {
                for old_term_id in old_term_ids {
                    self.offsets
                        .remove_offsets(&tx, doc_id, old_term_id)
                        .await?;
                }
            }
        }
        ...
        tx.set(
            ...
            VersionedStore::try_into(&self.state)?,
            None,
        )
        .await?;
        drop(tx);
    ...
    pub(super) async fn extract_querying_terms(
        ...
        let res = self
            .analyzer
            .extract_querying_terms(stk, ctx, opt, &t, query_string)
            .await?;
        drop(t);
        Ok(res)
    }
...
mod tests {
    ...
    async fn search(
        ...
        let (term_list, _) = fti
            .extract_querying_terms(stk, ctx, opt, qs.to_string())
            .await
            .unwrap();
        let tx = ctx.tx();
            ...
            stack
                .enter(|stk| async {
                    ...
                    check_hits(
                        ...
                        hits,
                        scr,
                        vec![(&doc1, Some(0.0)), (&doc2, Some(0.0))],
                    )
                    .await;
    ...
    async fn remove_insert_task(
        ...
        fti.index_document(stk, &ctx, &opt, rid, vec![content.clone()])
            .await
            .unwrap();
        finish(&ctx, fti).await;
    }
