            ...
            Some(p) => match self {
                ...
                Value::Array(v) => match p {
                    ...
                    _ => {
                        ...
                        stk.run(|stk| mapped.get(stk, ctx, opt, doc, path.skip(len)))
                            .await
                    }
                },
...
mod tests {
    ...
    async fn get_future_embedded_field_with_reference() {
        ...
        let mut stack = reblessive::tree::TreeStack::new();
        let res = stack
            .enter(|stk| val.get(stk, &ctx, &opt, Some(&cur), &idi))
            .finish()
            .await
            .unwrap();
        assert_eq!(
