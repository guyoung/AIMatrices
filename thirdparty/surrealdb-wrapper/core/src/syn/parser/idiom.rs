...
impl Parser<'_> {
    ...
    pub(super) async fn parse_fields(&mut self, ctx: &mut Stk) -> ParseResult<Fields> {
        if self.eat(t!("VALUE")) {
            ...
            Ok(Fields(vec![Field::Single { expr, alias }], true))
        } else {
...
mod tests {
    ...
    fn graph_conditions() {
        ...
        let out = Value::parse(sql);
        assert_eq!(
            "->(likes, follows WHERE influencer = true)",
            format!("{}", out)
        );
    }
    ...
    fn idiom_nested() {
        ...
        assert_eq!(
            out,
            Value::from(Idiom(vec![Part::from("test"), Part::from("temp")]))
        );
    }
    ...
    fn idiom_nested_quoted() {
        ...
        assert_eq!(
            out,
            Value::from(Idiom(vec![Part::from("test"), Part::from("some key")]))
        );
    }
