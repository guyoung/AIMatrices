            ...
            Some(op_val) => match self.get("path") {
                Some(path_val) => {
                    ...
                    match op_val.clone().as_string().as_str() {
                        ...
                        // Copy operation
                        "copy" => Ok(Operation::Copy { path, from: from? }),
                        // Move operation
                        "move" => Ok(Operation::Move { path, from: from? }),
                        // Test operation
