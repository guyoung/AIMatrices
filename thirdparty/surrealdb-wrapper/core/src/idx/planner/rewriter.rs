...
impl<'a> KnnConditionRewriter<'a> {
    ...
    fn eval_destructure_part(&self, part: &DestructurePart) -> Option<DestructurePart> {
        match part {
            DestructurePart::Aliased(f, v) => self
                .eval_idiom(v)
                .map(|v| DestructurePart::Aliased(f.clone(), v)),
            DestructurePart::Destructure(f, v) => self
                .eval_destructure_parts(v)
                .map(|v| DestructurePart::Destructure(f.clone(), v)),
            p => Some(p.clone()),
        }
    }
    ...
    fn eval_part(&self, p: &Part) -> Option<Part> {
        match p {
            ...
            Part::Recurse(r, Some(v)) => self
                .eval_idiom(v)
                .map(|v| Part::Recurse(r.to_owned(), Some(v))),
        }
    }
    ...
    fn eval_range(&self, r: &Range) -> Option<Range> {
        if let Some(beg) = self.eval_bound(&r.beg) {
            self.eval_bound(&r.end).map(|end| Range { beg, end })
        } else {
            None
        }
    }
    ...
    fn eval_id_range(&self, r: &IdRange) -> Option<IdRange> {
        if let Some(beg) = self.eval_id_bound(&r.beg) {
            self.eval_id_bound(&r.end).map(|end| IdRange { beg, end })
        } else {
            None
        }
    }
    ...
    fn eval_function(&self, f: &Function) -> Option<Function> {
        match f {
            Function::Normal(s, args) => self
                .eval_values(args)
                .map(|args| Function::Normal(s.clone(), args)),
            Function::Custom(s, args) => self
                .eval_values(args)
                .map(|args| Function::Custom(s.clone(), args)),
            Function::Script(s, args) => self
                .eval_values(args)
                .map(|args| Function::Script(s.clone(), args)),
            Function::Anonymous(p, args) => self
                .eval_values(args)
                .map(|args| Function::Anonymous(p.clone(), args)),
        }
    }
    ...
    fn eval_expression(&self, e: &Expression) -> Option<Expression> {
        match e {
            Expression::Unary { o, v } => self
                .eval_value(v)
                .map(|v| Expression::Unary { o: o.clone(), v }),
            Expression::Binary { l, o, r } => {
                if let Some(l) = self.eval_value(l) {
                    self.eval_value(r)
                        .map(|r| Expression::Binary { l, o: o.clone(), r })
                } else {
                    None
                }
            }
