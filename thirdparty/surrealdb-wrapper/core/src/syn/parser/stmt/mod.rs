    ...
    async fn parse_stmt_inner(&mut self, ctx: &mut Stk) -> ParseResult<Statement> {
        ...
        match token.kind {
            t!("ACCESS") => {
                ...
                self.pop_peek();
                ctx.run(|ctx| self.parse_access(ctx))
                    .await
                    .map(Statement::Access)
            }
            t!("ALTER") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_alter_stmt(ctx))
                    .await
                    .map(Statement::Alter)
            }
            t!("ANALYZE") => {
            ...
            t!("CREATE") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_create_stmt(ctx))
                    .await
                    .map(Statement::Create)
            }
            t!("DEFINE") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_define_stmt(ctx))
                    .await
                    .map(Statement::Define)
            }
            t!("DELETE") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_delete_stmt(ctx))
                    .await
                    .map(Statement::Delete)
            }
            t!("FOR") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_for_stmt(ctx))
                    .await
                    .map(Statement::Foreach)
            }
            t!("IF") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_if_stmt(ctx))
                    .await
                    .map(Statement::Ifelse)
            }
            t!("INFO") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_info_stmt(ctx))
                    .await
                    .map(Statement::Info)
            }
            t!("INSERT") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_insert_stmt(ctx))
                    .await
            ...
            t!("LIVE") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_live_stmt(ctx))
                    .await
                    .map(Statement::Live)
            }
            t!("OPTION") => {
            ...
            t!("RETURN") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_return_stmt(ctx))
                    .await
                    .map(Statement::Output)
            }
            t!("RELATE") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_relate_stmt(ctx))
                    .await
                    .map(Statement::Relate)
            }
            t!("REMOVE") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_remove_stmt(ctx))
                    .await
                    .map(Statement::Remove)
            }
            t!("SELECT") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_select_stmt(ctx))
                    .await
                    .map(Statement::Select)
            }
            t!("LET") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_let_stmt(ctx))
                    .await
                    .map(Statement::Set)
            }
            t!("SHOW") => {
            ...
            t!("THROW") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_throw_stmt(ctx))
                    .await
                    .map(Statement::Throw)
            }
            t!("UPDATE") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_update_stmt(ctx))
                    .await
                    .map(Statement::Update)
            }
            t!("UPSERT") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_upsert_stmt(ctx))
                    .await
