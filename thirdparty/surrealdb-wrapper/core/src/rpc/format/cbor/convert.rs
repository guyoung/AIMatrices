                ...
                match t {
                    ...
                    TAG_GEOMETRY_MULTIPOINT => match v.deref() {
                        Data::Array(v) => {
                            ...
                            Ok(Value::Geometry(Geometry::MultiPoint(MultiPoint::from(
                                points,
                            ))))
                        }
                        _ => Err("Expected a CBOR array with Geometry Point values"),
                    ...
                    TAG_GEOMETRY_MULTILINE => match v.deref() {
                        Data::Array(v) => {
                            ...
                            Ok(Value::Geometry(Geometry::MultiLine(MultiLineString::new(
                                lines,
                            ))))
                        }
                        _ => Err("Expected a CBOR array with Geometry Line values"),
    ...
    fn try_from(val: Value) -> Result<Self, &'static str> {
        match val {
            ...
            Value::Number(v) => match v {
                ...
                Number::Decimal(v) => Ok(Cbor(Data::Tag(
                    TAG_STRING_DECIMAL,
                    Box::new(Data::Text(v.to_string())),
                ))),
            },
...
impl TryFrom<Range> for Data {
    ...
    fn try_from(r: Range) -> Result<Data, &'static str> {
        fn encode(b: Bound<Value>) -> Result<Data, &'static str> {
            match b {
                Bound::Included(v) => Ok(Data::Tag(
                    TAG_BOUND_INCLUDED,
                    Box::new(Cbor::try_from(v)?.0),
                )),
                Bound::Excluded(v) => Ok(Data::Tag(
                    TAG_BOUND_EXCLUDED,
                    Box::new(Cbor::try_from(v)?.0),
                )),
...
impl TryFrom<Id> for Data {
    ...
    fn try_from(v: Id) -> Result<Data, &'static str> {
        match v {
            ...
            Id::Range(v) => Ok(Data::Tag(
                TAG_RANGE,
                Box::new(v.deref().to_owned().try_into()?),
            )),
            Id::Uuid(v) => Ok(Data::Tag(
                TAG_SPEC_UUID,
                Box::new(Data::Bytes(v.into_bytes().into())),
            )),
