...
impl Iterable {
    pub(super) async fn iterate(
        ...
        if self.iteration_stage_check(ctx) {
            ...
            if let Some(dis) = dis {
                let mut coll = ConcurrentDistinctCollector { coll, dis };
...
impl Collected {
    pub(super) async fn process(
        ...
        match self {
            ...
            Self::Relatable { f, v, w, o } => Self::process_relatable(opt, txn, f, v, w, o).await,
    ...
    async fn process_relatable(
        ...
        txn.check_ns_db_tb(opt.ns()?, opt.db()?, &v.tb, opt.strict)
            .await?;
        // Fetch the data from the store
    ...
    async fn process_yield(opt: &Options, txn: &Transaction, v: Table) -> Result<Processed, Error> {
        // Check that the table exists
        txn.check_ns_db_tb(opt.ns()?, opt.db()?, &v, opt.strict)
            .await?;
...
pub(super) trait Collector {
        ...
        if ctx.is_ok() {
            match iterable {
                ...
                Iterable::Relatable(f, v, w, o) => {
                    self.collect(Collected::Relatable { f, v, w, o }).await?
                }
            }
        }
        Ok(())
    ...
    async fn range_prepare(
        ...
        // Check that the table exists
        txn.check_ns_db_tb(opt.ns()?, opt.db()?, tb, opt.strict)
            .await?;
    ...
    async fn collect_index_items(
        ...
        // Check that the table exists
        ctx.tx()
            .check_ns_db_tb(opt.ns()?, opt.db()?, &table.0, opt.strict)
            .await?;
