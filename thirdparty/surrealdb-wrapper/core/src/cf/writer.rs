...
impl Writer {
    ...
    pub(crate) fn get(&self) -> Vec<PreparedWrite> {
        ...
        for (ChangeKey { ns, db, tb }, mutations) in self.buf.b.iter() {
...
mod tests {
    ...
    async fn changefeed_read_write() {
        ...
        ds.changefeed_process_at(ts.0.timestamp().try_into().unwrap())
            .await
            .unwrap();
        ...
        crate::cf::gc_range(&tx5, NS, DB, vs::u64_to_versionstamp(4))
            .await
            .unwrap();
        ...
        // Now we should see the gc_all results
        ds.changefeed_process_at((ts.0.timestamp() + 5).try_into().unwrap())
            .await
            .unwrap();
    ...
    async fn scan_picks_up_from_offset() {
        ...
        let vs1 = tx
            .get_versionstamp_from_timestamp(5, NS, DB)
            .await
            .unwrap()
            .unwrap();
        let vs2 = tx
            .get_versionstamp_from_timestamp(10, NS, DB)
            .await
            .unwrap()
            .unwrap();
    ...
    async fn set_with_diff_records_diff_to_achieve_original() {
        ...
        let res = ds
            .execute(format!("CREATE {thing} SET value=50").as_str(), &ses, None)
            .await
            .unwrap();
        ...
        // Now update it
        ds.changefeed_process_at((ts.0.timestamp() + 10).try_into().unwrap())
            .await
            .unwrap();
        let res = ds
            .execute(
