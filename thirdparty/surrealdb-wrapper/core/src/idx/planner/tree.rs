...
impl<'a> TreeBuilder<'a> {
    ...
    fn resolve_indexes(&mut self, t: &Table, i: &Idiom, schema: &SchemaCache) -> LocalIndexRefs {
        ...
        if let Some(e) = self.idioms_indexes.get_mut(t) {
            ...
            self.idioms_indexes
                .insert(t.clone(), HashMap::from([(i, irs.clone())]));
        }
        irs
    }
    ...
    async fn eval_expression(
        ...
        match e {
            Expression::Unary { .. } => {
                self.leaf_nodes_count += 1;
                Ok(Node::Unsupported(
                    "unary expressions not supported".to_string(),
                ))
            }
        ...
        for (irf, id_col) in irs.iter().filter(|(_, id_col)| 0.eq(id_col)) {
            if let Some(ix) = self.index_map.definitions.get(*irf as usize) {
                let op = match &ix.index {
                    ...
                    Index::Search { .. } => Self::eval_matches_operator(op, n),
    ...
    async fn eval_subquery(&mut self, stk: &mut Stk, s: &Subquery) -> Result<Node, Error> {
        ...
        match s {
            Subquery::Value(v) => {
                stk.run(|stk| self.eval_value(stk, self.group_sequence, v))
                    .await
            }
