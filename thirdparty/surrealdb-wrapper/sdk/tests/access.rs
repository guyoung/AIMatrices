...
async fn access_bearer_operations() {
    ...
    for level in &test_levels {
        ...
        let ses = match level.base {
            ...
            Base::Db => Session::owner()
                .with_ns(level.ns.unwrap())
                .with_db(level.db.unwrap()),
...
async fn access_bearer_revoke() {
    ...
    for level in &test_levels {
        ...
        let res = &mut dbs
            .execute(&format!("ACCESS srv REVOKE GRANT {kid}"), &ses, None)
            .await
            .unwrap();
        let tmp = res.remove(0).result.unwrap().to_string();
        ...
        let res = &mut dbs
            .execute("ACCESS srv REVOKE ALL", &ses, None)
            .await
            .unwrap();
        let tmp = res.remove(0).result.unwrap().to_string();
        let ok = Regex::new(
        ...
        let res = &mut dbs
            .execute("ACCESS srv SHOW WHERE revocation IS NONE", &ses, None)
            .await
            .unwrap();
        let tmp = res.remove(0).result.unwrap().to_string();
...
async fn access_bearer_show() {
    ...
    for level in &test_levels {
        ...
        let res = &mut dbs
            .execute(
                &format!("ACCESS srv SHOW WHERE grant.id = '{kid}'"),
                &ses,
                None,
            )
            .await
...
async fn access_bearer_purge() {
    ...
    for level in &test_levels {
        ...
        let res = &mut dbs
            .execute(&format!("ACCESS srv SHOW GRANT {kid}"), &ses, None)
            .await
            .unwrap();
        let tmp = res.remove(0).result.unwrap().to_string();
        let ok = Regex::new(&format!(
        ...
        let res = &mut dbs
            .execute("ACCESS srv PURGE REVOKED", &ses, None)
            .await
            .unwrap();
        let tmp = res.remove(0).result.unwrap().to_string();
        let ok = Regex::new(&format!(
        ...
        let res = &mut dbs
            .execute("ACCESS srv SHOW ALL", &ses, None)
            .await
            .unwrap();
        let tmp = res.remove(0).result.unwrap().to_string();
        let ok = Regex::new(
        ...
        let res = &mut dbs
            .execute("ACCESS srv PURGE EXPIRED FOR 2s", &ses, None)
            .await
            .unwrap();
        let tmp = res.remove(0).result.unwrap().to_string();
        let ok = Regex::new(r"\[\]").unwrap();
    ...
    for level in &test_levels {
        ...
        for ((level, role), (ns, db), should_succeed, msg) in tests.into_iter() {
            ...
            let sess_setup = match test_level {
                Base::Root => Session::for_level(().into(), Role::Owner)
                    .with_ns("NS")
                    .with_db("DB"),
                Base::Ns => Session::for_level(("NS",).into(), Role::Owner)
                    .with_ns("NS")
                    .with_db("DB"),
                Base::Db => Session::for_level(("NS", "DB").into(), Role::Owner)
                    .with_ns("NS")
                    .with_db("DB"),
                _ => panic!("Invalid base"),
            };
...

            {
                ...
                let mut resp = ds
                    .execute(&statement_setup, &sess_setup, None)
                    .await
                    .unwrap();
