...
pub async fn iam_run_case(
    ...
    if should_succeed {
        ...
        if res.is_ok() {
            return Err(format!(
                "Test statement succeeded when it should have failed: {:?}",
                res
            )
            .into());
        }
...
pub async fn iam_check_cases(
    ...
    for ((level, role), (ns, db), should_succeed) in cases {
        ...
        let sess = Session::for_level(level.to_owned(), role.to_owned())
            .with_ns(ns)
            .with_db(db);
...
impl Test {
    ...
    pub fn next(&mut self) -> Result<Response, Error> {
        assert!(
            ...
            "No response left - last position: {}",
            self.pos
        );
        self.pos += 1;
    ...
    pub fn expect_value_info<I: Display>(
        ...
        let val = if let Value::Constant(c) = val {
            c.compute()
                .unwrap_or_else(|e| panic!("Can't convert constant {c} - {e}"))
        } else {
            val
        };
        if val.is_nan() {
