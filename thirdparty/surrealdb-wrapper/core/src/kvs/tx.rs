...
impl Transaction {
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                let val = self
                    .get(key, None)
                    .await?
                    .ok_or_else(|| Error::UserRootNotFound {
                        value: us.to_owned(),
                    })?;
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                let val = self
                    .get(key, None)
                    .await?
                    .ok_or_else(|| Error::AccessRootNotFound { ac: ra.to_owned() })?;
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                let val =
                    self.get(key, None)
                        .await?
                        .ok_or_else(|| Error::AccessGrantRootNotFound {
                            ac: ac.to_owned(),
                            gr: gr.to_owned(),
                        })?;
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                let val = self
                    .get(key, None)
                    .await?
                    .ok_or_else(|| Error::UserNsNotFound {
                        value: us.to_owned(),
                        ns: ns.to_owned(),
                    })?;
                let val: DefineUserStatement = val.into();
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                let val =
                    self.get(key, None)
                        .await?
                        .ok_or_else(|| Error::AccessGrantNsNotFound {
                            ac: ac.to_owned(),
                            gr: gr.to_owned(),
                            ns: ns.to_owned(),
                        })?;
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                let val =
                    self.get(key, None)
                        .await?
                        .ok_or_else(|| Error::AccessGrantDbNotFound {
                            ac: ac.to_owned(),
                            gr: gr.to_owned(),
                            ns: ns.to_owned(),
                            db: db.to_owned(),
    ...
    pub async fn set_record(
        ...
        self.cache
            .insert(key.into(), cache::tx::Entry::Val(Arc::new(val)));
        // Return nothing
        Ok(())
    }
        ...
        match strict {
            ...
            true => {
                ...
                match self.get_tb(ns, db, tb).await {
                    Err(Error::TbNotFound { value: tb }) => {
                        // If not, check the database exists
                        match self.get_db(ns, db).await {
                            Err(Error::DbNotFound { value: db }) => {
                                // If not, check the namespace exists
                                match self.get_ns(ns).await {
                                    Err(Error::NsNotFound { value: ns }) => {
                                        Err(Error::NsNotFound { value: ns })
                                    }
                                    // Return any other errors
                                    Err(err) => Err(err),
                                    // Namespace does exist
                                    Ok(_) => Err(Error::DbNotFound { value: db }),
                                }
                            }
                            // Return any other errors
                            Err(err) => Err(err),
                            // Database does exist
                            Ok(_) => Err(Error::TbNotFound { value: tb }),
                        }
                    }
                    // Return any other errors
                    Err(err) => Err(err),
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                match res {
                    ...
                    Err(Error::NsNotFound { .. }) if !strict => {
    ...
    async fn get_or_add_db_upwards(
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                match res {
                    // Store a new default value in the datastore
                    Err(Error::DbNotFound { .. }) if !strict => {
                    ...
                    Err(Error::DbNotFound { value }) if strict => {
                        self.get_ns(ns).await?;
                        Err(Error::DbNotFound { value })?
                    }
                    // Store the fetched value in the cache
                    Ok(val) => {
                        let val: DefineDatabaseStatement = val.into();
    ...
    async fn get_or_add_tb_upwards(
        ...
        match self.cache.get(&qey) {
            ...
            None => {
                ...
                match res {
                    // Store a new default value in the datastore
                    Err(Error::TbNotFound { .. }) if !strict => {
                    ...
                    // Check to see that the hierarchy exists
                    Err(Error::TbNotFound { value }) if strict => {
                        self.get_ns(ns).await?;
                        self.get_db(ns, db).await?;
                        Err(Error::TbNotFound { value })?
                    }
                    // Store the fetched value in the cache
                    Ok(val) => {
                        let val: DefineTableStatement = val.into();
