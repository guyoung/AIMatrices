...
impl Building {
    ...
    async fn compute(&self) -> Result<(), Error> {
        ...
        while let Some(rng) = next {
            ...
            // Index the batch
                ...
                tx,
                self.index_initial_batch(&ctx, &tx, batch.values, &mut count)
                    .await
            );
            tx.commit().await?;
        }
        ...
        loop {
            ...
            let tx = ctx.tx();
                ...
                tx,
                self.index_appending_range(&ctx, &tx, range, &mut count)
                    .await
            );
            tx.commit().await?;
    ...
    async fn index_initial_batch(
        ...
        for (k, v) in values.into_iter() {
            ...
            *count += 1;
            self.set_status(BuildingStatus::InitialIndexing(*count))
                .await;
        }
        Ok(())
    }
    ...
    async fn index_appending_range(
        ...
        for i in range {
            ...
            if let Some(v) = tx.get(ia.clone(), None).await? {
                ...
                *count += 1;
                self.set_status(BuildingStatus::UpdatesIndexing(*count))
                    .await;
            }
        }
        Ok(())
