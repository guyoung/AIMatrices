...
pub fn parse_analyze() {
    ...
    assert_eq!(
        res,
        Statement::Analyze(AnalyzeStatement::Idx(
            Ident("a".to_string()),
            Ident("b".to_string())
        ))
    )
...
fn parse_define_database() {
    let res = test_parse!(
        parse_stmt,
        "DEFINE DATABASE a COMMENT 'test' CHANGEFEED 10m INCLUDE ORIGINAL"
    )
    .unwrap();
    assert_eq!(
        res,
...
fn parse_define_user() {
    ...
    // With roles.
    {
        ...
        assert_eq!(
            stmt.roles,
            vec![Ident("editor".to_string()), Ident("OWNER".to_string())]
        );
        assert_eq!(
            stmt.duration,
            UserDuration {
...
fn parse_define_access_record() {
    ...
    // Verification with JWT is explicitly defined only with symmetric key. Token duration is none.
    {
        let res = test_parse!(
            parse_stmt,
            r#"DEFINE ACCESS a ON DB TYPE RECORD DURATION FOR TOKEN NONE"#
        );
        assert!(
            res.is_err(),
    ...
    // Attempt to define record access at the namespace level.
    {
        let res = test_parse!(
            parse_stmt,
            r#"DEFINE ACCESS a ON NS TYPE RECORD DURATION FOR TOKEN NONE"#
        );
        assert!(
            res.is_err(),
...
fn parse_define_param() {
    let res = test_parse!(
        parse_stmt,
        r#"DEFINE PARAM $a VALUE { a: 1, "b": 3 } PERMISSIONS WHERE null"#
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        Statement::Define(DefineStatement::Table(DefineTableStatement {
            ...
            view: Some(crate::sql::View {
                ...
                cond: None,
                group: Some(Groups(vec![Group(Idiom(vec![Part::Field(Ident(
                    "foo".to_owned()
                ))]))])),
            }),
...
fn parse_define_event() {
    let res = test_parse!(
        parse_stmt,
        r#"DEFINE EVENT event ON TABLE table WHEN null THEN null,none"#
    )
    .unwrap();
...
fn parse_define_field() {
        ...
        assert_eq!(
            ...
            Statement::Define(DefineStatement::Field(DefineFieldStatement {
                ...
                kind: Some(Kind::Option(Box::new(Kind::Either(vec![
                    Kind::Number,
                    Kind::Array(
                        Box::new(Kind::Record(vec![Table("foo".to_owned())])),
                        Some(10)
                    )
                ])))),
                readonly: false,
    ...
    // Invalid DELETE permission
    {
        ...
        let res = test_parse!(
            parse_stmt,
            r#"DEFINE FIELD foo ON TABLE bar PERMISSIONS FOR DELETE NONE"#
        )
        .unwrap();
...
fn parse_define_index() {
    ...
    let res = test_parse!(
        parse_stmt,
        r#"DEFINE INDEX index ON TABLE table FIELDS a UNIQUE"#
    )
    .unwrap();
...
fn parse_delete() {
    ...
    assert_eq!(
        ...
        Statement::Delete(DeleteStatement {
            only: true,
            what: Values(vec![Value::Mock(crate::sql::Mock::Range(
                "foo".to_string(),
                32,
                64
            ))]),
...
fn parse_if() {
    let res = test_parse!(
        parse_stmt,
        r#"IF foo THEN bar ELSE IF faz THEN baz ELSE baq END"#
    )
    .unwrap();
    assert_eq!(
        res,
...
fn parse_if_block() {
    let res = test_parse!(
        parse_stmt,
        r#"IF foo { bar } ELSE IF faz { baz } ELSE { baq }"#
    )
    .unwrap();
    assert_eq!(
        res,
        Statement::Ifelse(IfelseStatement {
            ...
            close: Some(Value::Block(Box::new(Block(vec![Entry::Value(
                ident_field("baq")
            )])))),
        })
    )
...
fn parse_info() {
    ...
    assert_eq!(
        res,
        Statement::Info(InfoStatement::Tb(Ident("table".to_owned()), false, None))
    );

    let res = test_parse!(parse_stmt, "INFO FOR USER user").unwrap();
    assert_eq!(
        res,
        Statement::Info(InfoStatement::User(Ident("user".to_owned()), None, false))
    );

    let res = test_parse!(parse_stmt, "INFO FOR USER user ON namespace").unwrap();
    assert_eq!(
        res,
        Statement::Info(InfoStatement::User(
            Ident("user".to_owned()),
            Some(Base::Ns),
            false
        ))
...
SELECT bar as foo,[1,2],bar OMIT bar FROM ONLY a,1
    ...
    assert_eq!(
        ...
        Statement::Select(SelectStatement {
            ...
            omit: Some(Idioms(vec![Idiom(vec![Part::Field(Ident(
                "bar".to_owned()
            ))])])),
            only: true,
            what: Values(vec![
                Value::Table(Table("a".to_owned())),
                Value::Number(Number::Int(1))
            ]),
...
fn parse_insert_select() {
    let res = test_parse!(
        parse_stmt,
        r#"INSERT IGNORE INTO bar (select foo from baz)"#
    )
    .unwrap();
    assert_eq!(
        res,
...
fn parse_kill() {
    ...
    let res = test_parse!(
        parse_stmt,
        r#"KILL u"e72bee20-f49b-11ec-b939-0242ac120002" "#
    )
    .unwrap();
    assert_eq!(
        res,
...
fn parse_live() {
    ...
    let Statement::Live(stmt) = res else { panic!() };
    assert_eq!(stmt.expr, Fields::default());
    assert_eq!(stmt.what, Value::Param(Param(Ident("foo".to_owned()))));

    let res = test_parse!(
        parse_stmt,
        r#"LIVE SELECT foo FROM table WHERE true FETCH a[where foo],b"#
    )
    .unwrap();
    let Statement::Live(stmt) = res else { panic!() };
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(Fetchs(vec![
            Fetch(Value::Idiom(Idiom(vec![
                Part::Field(Ident("a".to_owned())),
                Part::Where(Value::Idiom(Idiom(vec![Part::Field(Ident(
                    "foo".to_owned()
                ))]))),
            ]))),
            Fetch(Value::Idiom(Idiom(vec![Part::Field(Ident(
                "b".to_owned()
            ))]))),
        ])),
    )
