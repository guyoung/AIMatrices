...
impl Terms {
    ...
    pub(super) async fn resolve_term_id(
        ...
        let term_key = term.into();
        {
            if let Some(term_id) = self
                .btree
                .search_mut(tx, &mut self.store, &term_key)
                .await?
            {
                return Ok(term_id);
            }
        ...
        tx.set(
            self.index_key_base.new_bu_key(term_id),
            term_key.clone(),
            None,
        )
        .await?;
        self.btree
            .insert(tx, &mut self.store, term_key, term_id)
            .await?;
        Ok(term_id)
    }
    ...
    pub(super) async fn remove_term_id(
        ...
        if let Some(term_key) = tx.get(term_id_key.clone(), None).await? {
            self.btree
                .delete(tx, &mut self.store, term_key.clone())
                .await?;
