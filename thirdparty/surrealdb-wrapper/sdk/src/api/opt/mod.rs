...
impl PatchOp {
    ...
    where
        T: Serialize,
    {
        Self(Serializer::new().serialize(InnerOp::Add { path, value }))
    }
    ...
    pub fn remove(path: &str) -> Self {
        Self(Serializer::new().serialize(UnitOp::Remove { path }))
    }
    ...
    where
        T: Serialize,
    {
        Self(Serializer::new().serialize(InnerOp::Replace { path, value }))
    }

    /// Changes a value
    pub fn change(path: &str, diff: String) -> Self {
        Self(Serializer::new().serialize(UnitOp::Change { path, value: diff }))
    }
}
