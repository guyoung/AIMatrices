...
async fn define_statement_index_concurrently_building_status() -> Result<(), Error> {
    ...
    loop {
        ...
        let mut r = ds
            .execute("INFO FOR INDEX test ON user", &session, None)
            .await?;
...
async fn define_statement_index_on_schemafull_without_permission() -> Result<(), Error> {
    ...
    assert!(
        tmp.is_err_and(|e| {
            if let Error::FdNotFound { value } = e {
...
async fn define_statement_search_index() -> Result<(), Error> {
    ...
    check_path(&tmp, &["doc_ids", "keys_count"], |v| {
        assert_eq!(v, Value::from(2))
    });
    check_path(&tmp, &["doc_ids", "max_depth"], |v| {
        assert_eq!(v, Value::from(1))
    });
    check_path(&tmp, &["doc_ids", "nodes_count"], |v| {
        assert_eq!(v, Value::from(1))
    });
    check_path(&tmp, &["doc_ids", "total_size"], |v| {
        assert_eq!(v, Value::from(62))
    });

    check_path(&tmp, &["doc_lengths", "keys_count"], |v| {
        assert_eq!(v, Value::from(2))
    });
    check_path(&tmp, &["doc_lengths", "max_depth"], |v| {
        assert_eq!(v, Value::from(1))
    });
    check_path(&tmp, &["doc_lengths", "nodes_count"], |v| {
        assert_eq!(v, Value::from(1))
    });
    check_path(&tmp, &["doc_lengths", "total_size"], |v| {
        assert_eq!(v, Value::from(56))
    });

    check_path(&tmp, &["postings", "keys_count"], |v| {
        assert_eq!(v, Value::from(17))
    });
    check_path(&tmp, &["postings", "max_depth"], |v| {
        assert_eq!(v, Value::from(1))
    });
    check_path(&tmp, &["postings", "nodes_count"], |v| {
        assert_eq!(v, Value::from(1))
    });
    check_path(&tmp, &["postings", "total_size"], |v| {
        assert!(v > Value::from(150))
    });

    check_path(&tmp, &["terms", "keys_count"], |v| {
        assert_eq!(v, Value::from(17))
    });
    check_path(&tmp, &["terms", "max_depth"], |v| {
        assert_eq!(v, Value::from(1))
    });
    check_path(&tmp, &["terms", "nodes_count"], |v| {
        assert_eq!(v, Value::from(1))
    });
    check_path(&tmp, &["terms", "total_size"], |v| {
        assert!(v.gt(&Value::from(150)))
    });
...
async fn permissions_checks_define_access_db() {
    let scenario = HashMap::from([
        ...
        (
            "test",
            "DEFINE ACCESS access ON DB TYPE JWT ALGORITHM HS512 KEY 'secret'",
        ),
        ("check", "INFO FOR DB"),
    ]);
