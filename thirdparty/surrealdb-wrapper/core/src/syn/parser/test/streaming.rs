...
fn test_streaming() {
    ...
    for i in 0..(source_bytes.len() - 1) {
            ...
            .finish()
        {
            ...
            PartialResult::Ok { value, used } => {
                assert_eq!(value, expected[current_stmt]);
                current_stmt += 1;
                source_start += used;
            }
            PartialResult::Err { err, .. } => {
                panic!(
                    "Streaming test returned an error: {}",
                    err.render_on_bytes(partial_source)
                )
            }
        }
    }
    ...
    match stack.enter(|stk| parser.parse_stmt(stk)).finish() {
        ...
        Err(e) => {
            panic!(
                "Streaming test returned an error: {}",
                e.render_on_bytes(partial_source)
            )
        }
    }
    ...
    assert_eq!(
        ...
        stack
            .enter(|stk| parser.parse_partial_statement(true, stk))
            .finish()
    );
}
