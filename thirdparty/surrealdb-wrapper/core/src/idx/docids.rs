...
impl DocIds {
    ...
    pub(in crate::idx) async fn resolve_doc_id(
        ...
        tx.set(
            self.index_key_base.new_bi_key(doc_id),
            doc_key.clone(),
            None,
        )
        .await?;
        self.btree
            .insert(tx, &mut self.store, doc_key, doc_id)
            .await?;
...
mod tests {
    ...
    async fn test_resolve_doc_id() {
        ...
        // Resolve another two existing doc keys and two new doc keys (interlaced)
        {
            let (tx, mut d) = new_operation(&ds, Write).await;
            assert_eq!(
                d.resolve_doc_id(&tx, "Foo".into()).await.unwrap(),
                Resolved::Existing(0)
            );
            assert_eq!(
                d.resolve_doc_id(&tx, "Hello".into()).await.unwrap(),
                Resolved::New(2)
            );
            assert_eq!(
                d.resolve_doc_id(&tx, "Bar".into()).await.unwrap(),
                Resolved::Existing(1)
            );
            assert_eq!(
                d.resolve_doc_id(&tx, "World".into()).await.unwrap(),
                Resolved::New(3)
            );
            finish(tx, d).await;
            let (tx, d) = new_operation(&ds, Read).await;
