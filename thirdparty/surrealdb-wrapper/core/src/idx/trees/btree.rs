    ...
    BK: BKeys + Debug + Clone,
{
    ...
    pub async fn insert(
        ...
        if let Some(root_id) = self.state.root {
            ...
            if root.n.keys().len() == self.full_size {
                ...
                let new_root = store.new_node(
                    new_root_id,
                    BTreeNode::Internal(BK::default(), vec![root_id]),
                )?;
                self.state.set_root(Some(new_root.id));
                self.split_child(store, new_root, 0, root).await?;
                self.insert_non_full(tx, store, new_root_id, key, payload)
                    .await?;
            } else {
                // The root node has place, let's insert the value
                let root_id = root.id;
                store.set_node(root, false).await?;
                self.insert_non_full(tx, store, root_id, key, payload)
                    .await?;
            }
        } else {
            // We don't have a root node, let's create id
            let new_root_id = self.state.new_node_id();
            let new_root_node = store.new_node(
                new_root_id,
                BTreeNode::Leaf(BK::with_key_val(key, payload)?),
            )?;
            #[cfg(debug_assertions)]
        ...
        while let Some(node) = next_node.take() {
            match &node.n {
                ...
                BTreeNode::Leaf(k) => {
                    let (key, payload) = k
                        .get_last_key()
                        .ok_or_else(|| fail!("BTree::find_highest(1)"))?;
                    #[cfg(debug_assertions)]
                    debug!(
                        "Find highest: {} - node: {}",
        ...
        while let Some(node) = next_node.take() {
            match &node.n {
                ...
                BTreeNode::Leaf(k) => {
                    let (key, payload) = k
                        .get_first_key()
                        .ok_or_else(|| fail!("BTree::find_lowest(1)"))?;
                    #[cfg(debug_assertions)]
                    debug!(
                        "Find lowest: {} - node: {}",
                        String::from_utf8_lossy(&key),
    ...
    async fn delete_adjust_predecessor(
        ...
        if left_child_stored_node
            .n
            .keys_mut()
            .remove(&ascending_key)
            .is_none()
        {
            #[cfg(debug_assertions)]
            panic!(
                "Remove key {} {}",
                String::from_utf8(ascending_key)?,
                left_child_stored_node.n
            );
        }
        let descending_key = keys
            .get_key(child_idx)
    ...
    async fn merge_nodes(
        ...
        let descending_key = keys
            .get_key(child_idx)
            .ok_or(Error::CorruptedIndex("BTree::merge_nodes(1)"))?;
        let descending_payload = keys
            .remove(&descending_key)
            .ok_or(Error::CorruptedIndex("BTree::merge_nodes(2)"))?;
        #[cfg(debug_assertions)]
        debug!(
            "descending_key: {}",
            String::from_utf8_lossy(&descending_key)
        );
        children.remove(child_idx + 1);
        let left_id = left_child.id;
        if left_child
            .n
            .append(descending_key, descending_payload, right_child.n)?
            .is_some()
        {
            #[cfg(debug_assertions)]
...
mod tests {
    ...
    async fn clrs_insertion_test() {
        ...
        assert_eq!(
            10,
            tx.scan(vec![]..vec![0xf], 100, None).await.unwrap().len()
        );
        ...
        let n = t
            .inspect_nodes_mut(tx, st, |_, _, _, sn| {
                ...
                for i in 0..keys.len() {
                    ...
                    if unique_keys
                        .insert(String::from_utf8(key).unwrap(), payload)
                        .is_some()
                    {
                        panic!("Non unique");
                    }
        ...
        BK: BKeys,
    {
        assert_eq!(
            keys.len() as usize,
            expected_keys.len(),
            "The number of keys does not match"
        );
        for (key, payload) in expected_keys {
            assert_eq!(
                keys.get(&key.into()),
