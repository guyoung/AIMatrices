...
impl Parser<'_> {
    ...
    async fn parse_object_or_geometry_after_type(
        ...
        match type_value.as_str() {
            ...
            _ => {
                ...
                if self.eat(t!(",")) {
                    self.parse_object_from_map(ctx, object, start)
                        .await
                        .map(Value::Object)
                } else {
                    self.expect_closing_delimiter(t!("}"), start)?;
    ...
    async fn parse_object_or_geometry(&mut self, ctx: &mut Stk, start: Span) -> ParseResult<Value> {
        ...
        match key.as_str() {
            "type" => {
                self.parse_object_or_geometry_after_type(ctx, start, key)
                    .await
            }
            "coordinates" => {
                self.parse_object_or_geometry_after_coordinates(ctx, start, key)
                    .await
            }
            "geometries" => {
                self.parse_object_or_geometry_after_geometries(ctx, start, key)
                    .await
            }
            _ => {
                expected!(self, t!(":"));
