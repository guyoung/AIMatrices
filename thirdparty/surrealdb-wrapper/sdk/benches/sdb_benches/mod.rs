...
static NUM_OPS: LazyLock<usize> = LazyLock::new(|| {
    std::env::var("BENCH_NUM_OPS")
        .unwrap_or("1000".to_string())
        .parse()
        .unwrap()
});
static DURATION_SECS: LazyLock<u64> = LazyLock::new(|| {
...
pub(super) fn benchmark_group(c: &mut Criterion, target: String) {
    ...
    match &target {
        ...
        t if t.starts_with("extension-base") => sdk::benchmark_group(c, target),
