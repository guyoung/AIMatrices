...
async fn database_change_feeds() -> Result<(), Error> {
    ...
    let second_timestamp = first_timestamp.flat_map(|vs1| {
        generate_versionstamp_sequences(vs1)
            .skip(1)
            .take(variance)
            .map(move |vs2| (vs1, vs2))
    });
...
async fn changefeed_with_ts() -> Result<(), Error> {
    ...
    db.changefeed_process_at(ts1.timestamp().try_into().unwrap())
        .await
        .unwrap();
    ...
    db.changefeed_process_at(ts2.timestamp().try_into().unwrap())
        .await
        .unwrap();
    //
    ...
    db.changefeed_process_at(ts3.timestamp().try_into().unwrap())
        .await
        .unwrap();
    //
...
async fn changefeed_with_original() -> Result<(), Error> {
    ...
    .remove(0)
    .result?;
    db.execute("CREATE user CONTENT {'id': 'id_one'};", &ses, None)
        .await?
        .remove(0)
        .result?;
    ...
    let value: Value = db
        .execute("SHOW CHANGES FOR TABLE user SINCE 0", &ses, None)
        .await?
        .remove(0)
        .result?;
    ...
    db.execute("UPDATE user:id_one SET name = 'Raynor';", &ses, None)
        .await?
        .remove(0)
        .result?;
    let array = db
