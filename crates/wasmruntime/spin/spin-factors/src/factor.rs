...
use crate::{prepare::FactorInstanceBuilder, Error, PrepareContext, RuntimeFactors};
...
pub struct ConfigureAppContext<'a, T: RuntimeFactors, F: Factor> {
    ...
    runtime_config: Option<Arc<F::RuntimeConfig>>,
}
...
impl<'a, T: RuntimeFactors, F: Factor> ConfigureAppContext<'a, T, F> {
    ...
    pub fn runtime_config(&self) -> Option<&Arc<F::RuntimeConfig>> {
        self.runtime_config.as_ref()
    }

    /// Take ownership of the runtime configuration for the given factor.
    pub fn take_runtime_config(&mut self) -> Option<Arc<F::RuntimeConfig>> {
