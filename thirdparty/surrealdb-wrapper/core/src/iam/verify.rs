...
mod tests {
    ...
    async fn test_basic_nonexistent_role() {
        ...
        for level in &test_levels {
            ...
            match res {
                ...
                res => {
                    panic!(
                        "Expected an invalid role IAM error, but instead received: {:?}",
                        res
                    )
                }
            }
    ...
    async fn test_token_record_custom_claims() {
        ...
        let later = (Utc::now() + Duration::hours(1)).timestamp();
        {
            ...
            test_object.insert(
                "test_2".to_string(),
                Value::Object(test_object_child.into()),
            );
    ...
    async fn test_token_record_jwks() {
        ...
        //
        {
            ...
            assert_eq!(
                sess.exp, None,
                "Default session expiration is expected to be None"
            );
        }
        ...
        //
        {
            ...
            assert!(
                res.is_err(),
                "Unexpected success signing in with token: {:?}",
                res
            );
        }
    }
    ...
    async fn test_verify_creds_invalid() {
        ...
        {
            assert!(verify_db_creds(&ds, &ns, &db, "test", "test")
                .await
                .is_err());
        }
    }
    ...
    async fn test_expired_token() {
        ...
        match res {
            ...
            res => panic!(
                "Unexpected success signing in with expired token: {:?}",
                res
            ),
        }
    }
