...
impl Request {
    ...
    fn parse_uri(uri: String) -> (Option<http::Uri>, String) {
        (
            http::Uri::try_from(&uri)
                .or_else(|_| http::Uri::try_from(&format!("http://{uri}")))
                .ok(),
            uri,
        )
    }
...
impl IncomingResponse {
    ...
    // `IncomingRequest::into_body_stream`).  However, as of this writing, `wasmtime-wasi-http_internal` is implemented in
    ...
    pub fn into_body_sync(&self) -> Result<Vec<u8>, streams::Error> {
        let body = self.consume().expect("response body was already consumed");
...
#[doc(hidden)]
/// The executor for driving wasi-http_internal futures to completion
mod executor;
#[doc(hidden)]
