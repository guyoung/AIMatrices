...
impl Datastore {
    ...
    pub(crate) async fn new(path: &str) -> Result<Datastore, Error> {
        match create_connection_pool(path, Some(*cnf::SURREALCS_CONNECTION_POOL_SIZE)).await {
            Ok(_) => Ok(Datastore {}),
            Err(_) => Err(Error::Ds(
                "Cannot connect to the `surrealcs` storage engine".to_string(),
            )),
        }
    }
...
impl super::api::Transaction for Transaction {
        ...
        V: Into<Val> + Debug,
    {
        ...
        let prep = if self.save_points.is_some() {
            self.save_point_prepare(&key, version, SaveOperation::Set)
                .await?
        } else {
            None
        };
        ...
        V: Into<Val> + Debug,
    {
        ...
        let prep = if self.save_points.is_some() {
            self.save_point_prepare(&key, version, SaveOperation::Put)
                .await?
        } else {
            None
        };
        ...
        V: Into<Val> + Debug,
    {
        ...
        // Hydrate the savepoint if any
        let prep = if self.save_points.is_some() {
            self.save_point_prepare(&key, None, SaveOperation::Put)
                .await?
        } else {
            None
        };
        ...
        K: Into<Key> + Sprintable + Debug,
    {
        ...
        // Hydrate the savepoint if any
        let prep = if self.save_points.is_some() {
            self.save_point_prepare(&key, None, SaveOperation::Del)
                .await?
        } else {
            None
        };
        // Remove the key
        let message = ServerTransactionMessage::Del(MessageDel { key });
        self.send_message(message).await?;
        // Confirm the save point
        if let Some(prep) = prep {
            self.save_points.save(prep);
