...
fn glued_identifiers() {
    ...
    let expected = sql::Query(sql::Statements(vec![Statement::Value(Value::Thing(
        Thing {
            tb: "T".to_string(),
            id: Id::Number(1),
        },
...
fn query_object() {
    ...
    test_parse!(parse_query, src)
        .inspect_err(|e| eprintln!("{}", e.render_on(src)))
        .unwrap();
}
...
fn ident_is_field() {
    ...
    let field = test_parse!(parse_query, src)
        .inspect_err(|e| eprintln!("{}", e.render_on(src)))
        .unwrap();
    assert_eq!(
        field,
        Query(Statements(vec![Statement::Value(Value::Idiom(Idiom(
            vec![Part::Field(Ident("foo".to_string()))]
        )))]))
    );
}
...
fn parse_immediate_insert_subquery() {
    test_parse!(
        parse_query,
        r#"LET $insert = INSERT INTO t (SELECT true FROM 1);"#
    )
    .unwrap();
}
