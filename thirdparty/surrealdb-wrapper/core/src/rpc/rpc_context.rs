...
pub trait RpcContext {
    ...
    async fn execute_mutable(&mut self, method: Method, params: Array) -> Result<Data, RpcError> {
        ...
        if !self.kvs().allows_rpc_method(&MethodTarget { method }) {
            warn!(
                "Capabilities denied RPC method call attempt, target: '{}'",
                method.to_str()
            );
    ...
    async fn signup(&mut self, params: Array) -> Result<Data, RpcError> {
        ...
        let out: Result<Value, Error> = crate::iam::signup::signup(self.kvs(), &mut session, v)
            .await
            .map(Into::into);
