...
pub enum Value {
    ...
    #[revision(
        ...
        convert_fn = "convert_old_range",
        fields_name = "OldValueRangeFields"
    )]
    Range(OldRange),
    #[revision(start = 2)]
    Range(Box<Range>),
...
impl From<IdRange> for Value {
    fn from(v: IdRange) -> Self {
        ...
        Value::Range(Box::new(Range { beg, end }))
    }
}
...
impl Value {
    ...
    pub fn is_geometry_type(&self, types: &[String]) -> bool {
        match self {
            Value::Geometry(Geometry::Point(_)) => types
                .iter()
                .any(|t| matches!(t.as_str(), "feature" | "point")),
            Value::Geometry(Geometry::Line(_)) => types
                .iter()
                .any(|t| matches!(t.as_str(), "feature" | "line")),
            Value::Geometry(Geometry::Polygon(_)) => types
                .iter()
                .any(|t| matches!(t.as_str(), "feature" | "polygon")),
            Value::Geometry(Geometry::MultiPoint(_)) => types
                .iter()
                .any(|t| matches!(t.as_str(), "feature" | "multipoint")),
            Value::Geometry(Geometry::MultiLine(_)) => types
                .iter()
                .any(|t| matches!(t.as_str(), "feature" | "multiline")),
            Value::Geometry(Geometry::MultiPolygon(_)) => types
                .iter()
                .any(|t| matches!(t.as_str(), "feature" | "multipolygon")),
            Value::Geometry(Geometry::Collection(_)) => types
                .iter()
                .any(|t| matches!(t.as_str(), "feature" | "collection")),
            _ => false,
        }
    }
    ...
    pub(crate) fn coerce_to(self, kind: &Kind) -> Result<Value, Error> {
            ...
            Kind::Either(k) => {
                ...
                for k in k {
                    match val.coerce_to(k) {
                        Err(Error::CoerceTo { from, .. }) => val = from,
                        Err(e) => return Err(e),
                        Ok(v) => return Ok(v),
                    }
                }
        ...
        match res {
            ...
            Err(Error::CoerceTo { from, .. }) => Err(Error::CoerceTo {
                from,
                into: kind.to_string(),
            }),
    ...
    pub(crate) fn coerce_to_array_type(self, kind: &Kind) -> Result<Array, Error> {
        self.coerce_to_array()?
            ...
            .map_err(|e| match e {
                Error::CoerceTo { from, .. } => Error::CoerceTo {
                    from,
    ...
    pub(crate) fn convert_to(self, kind: &Kind) -> Result<Value, Error> {
            ...
            Kind::Either(k) => {
                ...
                for k in k {
                    match val.convert_to(k) {
                        Err(Error::ConvertTo { from, .. }) => val = from,
                        Err(e) => return Err(e),
                        Ok(v) => return Ok(v),
                    }
                }
        ...
        match res {
            // There was a conversion error
            Err(Error::ConvertTo { from, .. }) => Err(Error::ConvertTo {
                from,
                into: kind.to_string(),
            }),
            // There was a different error
    ...
    pub(crate) fn convert_to_array_type(self, kind: &Kind) -> Result<Array, Error> {
        self.convert_to_array()?
            ...
            .collect::<Result<Array, Error>>()
            .map_err(|e| match e {
                Error::ConvertTo { from, .. } => Error::ConvertTo {
                    from,
                    into: format!("array<{kind}>"),
                },
                e => e,
...
mod tests {
    ...
    fn convert_string() {
        ...
        assert_eq!(
            ...
            Value::from("something").as_string()
        );
    }
    ...
    fn check_size() {
        ...
        assert_eq!(
            8,
            std::mem::size_of::<Box<crate::sql::function::Function>>()
        );
        assert_eq!(
            8,
            std::mem::size_of::<Box<crate::sql::subquery::Subquery>>()
        );
        assert_eq!(
            8,
            std::mem::size_of::<Box<crate::sql::expression::Expression>>()
        );
    }
