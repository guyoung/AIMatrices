...
fn bench_hnsw_no_db(c: &mut Criterion) {
    ...
    let samples: Vec<(Thing, Vec<Value>)> = samples
        .into_iter()
        .map(|(r, a)| (r, vec![Value::Array(a)]))
        .collect();
    ...
    let samples: Vec<Vec<Number>> = samples
        .into_iter()
        .map(|(_, a)| convert_array_to_vec_number(a))
        .collect();
    ...
    // Knn lookup benchmark group
    {
        ...
        group.bench_function(id, |b| {
            b.to_async(Runtime::new().unwrap())
                .iter(|| knn_lookup_objects(&ds, &hnsw, &samples));
        });
        group.finish();
    }
}
...
fn bench_hnsw_with_db(c: &mut Criterion) {
    ...
    let samples: Vec<String> = samples
        .into_iter()
        .map(|(r, a)| format!("CREATE {r} SET r={a} RETURN NONE;"))
        .collect();
    ...
    // Indexing benchmark group
    {
        ...
        group.bench_function(id, |b| {
            b.to_async(Runtime::new().unwrap())
                .iter(|| insert_objects_db(session, true, &samples));
        });
        group.finish();
    }
...
async fn hnsw(tx: &Transaction) -> HnswIndex {
    ...
    HnswIndex::new(tx, IndexKeyBase::default(), "test".to_string(), &p)
        .await
        .unwrap()
}
...
async fn insert_objects(samples: &[(Thing, Vec<Value>)]) -> (Datastore, HnswIndex) {
    ...
    for (thg, content) in samples {
        h.index_document(&tx, thg.id.clone(), content)
            .await
            .unwrap();
    }
