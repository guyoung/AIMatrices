...
impl AlterTableStatement {
    pub(crate) async fn compute(
        ...
        let mut dt = match txn.get_tb(opt.ns()?, opt.db()?, &self.name).await {
            ...
            Err(Error::TbNotFound { .. }) if self.if_exists => return Ok(Value::None),
            Err(v) => return Err(v),
        };
        ...
        if self.changefeed.is_some() && dt.changefeed.is_some() {
            txn.lock()
                .await
                .record_table_change(opt.ns()?, opt.db()?, &self.name, &dt);
        }
        // Clear the cache
        txn.clear();
        // Ok all good
...
impl Display for AlterTableStatement {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(full) = self.full {
            f.write_str(if full { " SCHEMAFULL" } else { " SCHEMALESS" })?;
        }
        ...
        if let Some(changefeed) = &self.changefeed {
            write!(
                f,
                " CHANGEFEED {}",
                changefeed.map_or("NONE".into(), |v| v.to_string())
            )?
        }
