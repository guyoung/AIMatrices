...
impl Parser<'_> {
    pub(crate) async fn parse_record_string(
        ...
        debug_assert!(self
            .last_span()
            .is_followed_by(&self.peek_whitespace().span));
    ...
    pub(crate) async fn parse_thing_or_range(
        ...
        if self.eat_whitespace(t!("..")) {
            ...
            Ok(Thing {
                tb: ident,
                id: Id::Range(Box::new(IdRange { beg, end })),
            })
        } else {
            let id = match beg {
                Bound::Unbounded => {
...
mod tests {
    ...
    fn thing_string() {
        ...
        let Value::Thing(out) = res else { panic!() };
        assert_eq!("test:1", format!("{}", out));
        assert_eq!(
            out,
            Thing {
    ...
    fn weird_things() {
        ...
        fn assert_ident_parses_correctly(ident: &str) {
            ...
            assert_eq!(
                r,
                sql::Query(sql::Statements(vec![sql::Statement::Value(
                    sql::Value::Thing(sql::Thing {
                        tb: "t".to_string(),
