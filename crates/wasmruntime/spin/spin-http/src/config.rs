...
pub struct HttpTriggerConfig {
    ...
    /// The HTTP spin-executor the component requires
    #[serde(default)]
...
/// The spin-executor for the HTTP component.
/// The component can either implement the Spin HTTP interface,
/// the `wasi-http` interface, or the Wagi CGI interface.
///
/// If an spin-executor is not specified, the inferred default is `HttpExecutor::Spin`.
#[derive(Clone, Debug, Default, Deserialize, Serialize)]
...
/// Wagi specific configuration for the http spin-executor.
#[derive(Clone, Debug, Deserialize, Serialize)]
