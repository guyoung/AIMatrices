...
impl IndexEqualThingIterator {
    ...
    async fn next_scan<B: IteratorBatch>(
        ...
        res.into_iter()
            .for_each(|(_, val)| records.add((Arc::new(val.into()), irf.into(), None)));
        Ok(records)
    }
...
impl RangeScan {
    fn new(beg: Vec<u8>, beg_incl: bool, end: Vec<u8>, end_incl: bool) -> Self {
        let beg_excl = if !beg_incl { Some(beg.clone()) } else { None };
        let end_excl = if !end_incl { Some(end.clone()) } else { None };
        Self {
            beg,
            end,
            beg_excl,
...
impl IndexJoinThingIterator {
    pub(super) fn new(
        ...
        Ok(Self(
            irf,
            JoinThingIterator::new(opt, ix, remote_iterators)?,
        ))
    }
...
impl MatchesThingIterator {
    pub(super) async fn new(
        ...
        let hits_left = if let Some(h) = &hits { h.len() } else { 0 };
        Ok(Self {
            irf,
            hits,
            hits_left,
