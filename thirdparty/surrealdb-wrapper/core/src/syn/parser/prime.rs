...
impl Parser<'_> {
    ...
    pub(super) async fn try_parse_inline(
        ...
        if self.eat_whitespace(t!("(")) {
            ...
            let value = ctx
                .run(|ctx| self.try_parse_inline(ctx, &value))
                .await?
                .unwrap_or(value);
            Ok(Some(value))
        } else {
    ...
    pub(super) async fn parse_idiom_expression(&mut self, ctx: &mut Stk) -> ParseResult<Value> {
        ...
        let value = match token.kind {
            ...
            | t!("-")
            | TokenKind::Digits
            | TokenKind::Glued(Glued::Number | Glued::Duration) => {
            ...
            t!("||") => {
                self.pop_peek();
                ctx.run(|ctx| self.parse_closure_after_args(ctx, Vec::new()))
                    .await?
            }
            t!("IF") => {
            ...
            t!("(") => {
                self.pop_peek();
                let value = self
                    .parse_inner_subquery_or_coordinate(ctx, token.span)
                    .await?;
                self.try_parse_inline(ctx, &value).await?.unwrap_or(value)
            }
            t!("/") => self.next_token_value().map(Value::Regex)?,
            ...
            t!("fn") => {
                self.pop_peek();
                self.parse_custom_function(ctx)
                    .await
                    .map(|x| Value::Function(Box::new(x)))?
            }
            t!("ml") => {
                self.pop_peek();
                self.parse_model(ctx)
                    .await
                    .map(|x| Value::Model(Box::new(x)))?
            }
            x if Self::kind_is_identifier(x) => {
        ...
        if self.peek_continues_idiom() {
            let value = match value {
                ...
                x => {
                    self.parse_remaining_value_idiom(ctx, vec![Part::Start(x)])
                        .await
                }
            }?;
            Ok(self.try_parse_inline(ctx, &value).await?.unwrap_or(value))
...
mod tests {
    ...
    fn regex_simple() {
        ...
        let Value::Regex(regex) = out else { panic!() };
