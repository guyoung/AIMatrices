...
impl MTreeIndex {
    ...
    pub async fn index_document(
        ...
        for v in content.iter().filter(|v| v.is_some()) {
            ...
            mtree
                .insert(stk, txn, &mut self.store, vector.into(), doc_id)
                .await?;
        }
        drop(mtree);
        Ok(())
    ...
    pub async fn remove_document(
        ...
        if let Some(doc_id) = doc_id {
            ...
            for v in content.iter().filter(|v| v.is_some()) {
                ...
                // Remove the vector
                mtree
                    .delete(stk, txn, &mut self.store, vector.into(), doc_id)
                    .await?;
            }
            drop(mtree);
        }
    ...
    pub async fn finish(&mut self, tx: &Transaction) -> Result<(), Error> {
        ...
        if let Some(new_cache) = self.store.finish(tx).await? {
            ...
            tx.set(
                ...
                VersionedStore::try_into(&mtree.state)?,
                None,
            )
            .await?;
...
impl MTree {
    ...
    async fn insert(
        ...
        if let Some(root_id) = self.state.root {
            ...
            if let InsertionResult::PromotedEntries(o1, p1, o2, p2) = self
                .insert_at_node(stk, tx, store, node, &None, obj, id)
                .await?
            {
    ...
    async fn delete_node_internal(
        ...
        #[cfg(debug_assertions)]
        debug!(
            "delete_node_internal ID: {} - DocID: {} - obj: {:?}",
            node_id, id, od
        );
        ...
        for (on_obj, on_entry) in &n_node {
            ...
            debug!(
                "on_od_dist: {:?} / {} / {}",
                on_obj, on_od_dist, on_entry.radius
            );
            // If (d(Od, On) <= r(On))
    ...
    async fn set_stored_node(
        ...
        store
            .set_node(StoredNode::new(node, node_id, node_key, 0), updated)
            .await?;
        Ok(())
    }
    ...
    async fn deletion_underflown(
        ...
        #[cfg(debug_assertions)]
        debug!(
            "deletion_underflown - p_id: {} - onn: {:?} - n_len: {}",
            p.id,
            onn,
            n_node.len()
        );
    ...
    async fn delete_underflown_redistribute(
        ...
        #[cfg(debug_assertions)]
        debug!(
            "deletion_underflown - delete_underflown_redistribute Node ID: {}",
            p.id
        );
...
impl NodeVectors for LeafNode {
    ...
    fn extract_node(
        ...
        for o in a {
            let mut props = self
                .remove(&o)
                .ok_or_else(|| fail!("NodeVectors/LeafNode::extract_node)"))?;
...
impl NodeVectors for InternalNode {
    ...
    fn extract_node(
        ...
        for o in a {
            let mut props = self
                .remove(&o)
                .ok_or_else(|| fail!("NodeVectors/InternalNode::extract_node"))?;
            let dist = *distances.0.get(&(o.clone(), p.clone())).unwrap_or(&0f64);
...
mod tests {
    ...
    async fn delete_collection(
        ...
        if all_deleted {
            let (ctx, mut st) = new_operation(ds, t, TransactionType::Read, cache_size).await;
            let tx = ctx.tx();
            check_tree_properties(&tx, &mut st, t)
                .await?
    ...
    impl CheckedProperties {
        fn check(
            ...
            assert_eq!(
                self.min_leaf_depth, expected_leaf_depth,
                "Min leaf depth - {:?}",
                self
            );
            assert_eq!(
                self.max_leaf_depth, expected_leaf_depth,
                "Max leaf depth - {:?}",
                self
            );
            assert_eq!(
                self.min_objects, expected_min_objects,
                "Min objects - {:?}",
                self
            );
            assert_eq!(
                self.max_objects, expected_max_objects,
                "Max objects - {:?}",
                self
            );
            assert_eq!(
                self.object_count, expected_object_count,
                "Object count- {:?}",
                self
            );
